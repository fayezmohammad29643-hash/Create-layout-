...
def validate_source_integrity(doc_path: Path, sqlite_path: Path, config: dict) -> dict:
    """Refuse to build when the DOC/SQLite pair is not the configured edition.

    Warsh and Douri have genuinely different ayah segmentation/counts. The
    builder therefore must never silently accept a different SQLite database
    under the same filename. Exact SHA-256 fingerprints are used for the
    versioned sources in this project.
    """
    ...
    if expected_doc and actual_doc != expected_doc:
        raise RuntimeError(
            f"Source DOC integrity mismatch for {doc_path.name}: "
            f"expected SHA256={expected_doc}, actual SHA256={actual_doc}. "
            "This file does not match the configured Quran edition."
        )
    if expected_sqlite and actual_sqlite != expected_sqlite:
        raise RuntimeError(
            f"Source SQLite integrity mismatch for {sqlite_path.name}: "
            f"expected SHA256={expected_sqlite}, actual SHA256={actual_sqlite}. "
            f"The configured {config.get('qiraah', 'unknown')} workflow cannot use a different edition/riwayah database."
        )
