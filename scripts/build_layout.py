...
def make_line(surah_id: int, line_number: int, line_text: str, paragraph_props: dict, runs: list[dict], ayah: int, word_index: int) -> tuple[dict | None, int, int]:
    """Create the compact schema-v3 line record.

    The DOCX remains the source of line boundaries. The generated layout stores
    only the first and last word/ayah-marker references; Quran text is expected
    to come from the separate SQLite Quran database.
    """
    line_text = strip_meta(line_text)
    if not line_text.strip():
        return None, ayah, word_index
    endpoints, new_ayah, new_word_index, _verses = build_endpoints(
        surah_id, ayah, word_index, line_text
    )
...
def extract_docx(docx_path: Path, forced_font: str) -> dict:
    ...
    for page_no in sorted(pages):
        ...
        for line in page_obj["lines"]:
            for endpoint_key in ("first_word", "end_word"):
                endpoint = line.get(endpoint_key, {})
                sid = endpoint.get("surah")
    ...
    # Rebuild per-surah line buckets from pages. A line belongs to a surah if
    # either endpoint refers to that surah.
    for sid, surah in surahs.items():
        bucket = []
        for page_no in sorted(surah["pages"]):
            lines = []
            for line in pages[page_no]["lines"]:
                endpoints = (line.get("first_word", {}), line.get("end_word", {}))
                if any(endpoint.get("surah") == sid for endpoint in endpoints):
                    lines.append(line)
            if lines:
        ...
        for p in bucket:
            for line in p["lines"]:
                for endpoint in (line.get("first_word", {}), line.get("end_word", {})):
...
def write_outputs(result: dict, output_dir: Path) -> None:
    ...
    for page_no in range(1, result["total_pages"] + 1):
        page_data = result["pages"].get(page_no, {"elements": [], "lines": [], "surah_ids": []})
        payload = {
            "page": page_no,
            "surah_ids": page_data["surah_ids"],
            "elements": page_data["elements_order"],
    ...
    for sid in range(1, result["total_surahs"] + 1):
        ...
        payload = {
            "schema_version": result["schema_version"],
            **{k: v for k, v in surah.items() if k != "pages"},
            "font_family": result["font_family"],
            "layout_source": "docx",
            "pages": surah["pages"],
        }
