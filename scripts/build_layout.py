...
def extract_svg_coordinates(
    svg_paths: Dict[int, Path], expected_lines_by_page: Dict[int, int]
) -> Dict[int, dict]:
    ...
    for page, svg_path in sorted(svg_paths.items()):
        try:
            results[page] = extract_lines.extract(
                svg_path, expected_line_count=expected_lines_by_page.get(page)
            )
        except Exception as exc:  # noqa: BLE001
            failures.append((page, str(exc)))
    if failures:
        ...
        for page_obj in surah["pages"]:
            ...
            if missing:
                raise RuntimeError(
                    ...
                    "The adaptive SVG line recovery could not resolve this page."
                )
...
def main() -> int:
    ...
    expected_lines_by_page: Dict[int, int] = {}
    for surah in surahs:
        for page_obj in surah["pages"]:
            page = int(page_obj["page"])
            expected_lines_by_page[page] = expected_lines_by_page.get(page, 0) + len(page_obj["lines"])
    ...
    with tempfile.TemporaryDirectory(prefix="quranpedia_svg_") as temp_name:
        ...
        svg_results = extract_svg_coordinates(svg_paths, expected_lines_by_page)
