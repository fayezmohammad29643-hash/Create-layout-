...
def solve_page(
    ...
    if direct_safe:
        ...
        if abs(total_delta) > max_abs_page_delta:
            raise RuntimeError(
                "PDF/SQLite visual word-count mismatch is too large for safe correction: "
...
def main() -> None:
    ...
    max_visual_count_delta = int(tok_cfg.get("max_visual_count_delta_for_correction", 3))
    ...
    for page_no in range(1, expected_pages + 1):
        ...
        counts, diagnostics = solve_page(
            pdf_lines, tokens, max_words_per_line, candidate_extra, max_visual_count_delta
        )
    ...
    manifest = {
        ...
        "layout_source": "pdf",
        "pdf_alignment_version": 3,
        "word_boundary_method": "rawdict_visual_line_units_exact_or_constrained_dp",
