...
from build_doc_layout import canonical, split_token, sha256_file  # noqa: E402
...
def main():
    ...
    ap.add_argument("--doc", type=Path, required=False)
    args = ap.parse_args()
    ...
    manifest_sqlite_hash = source.get("sqlite_sha256")
    if manifest_sqlite_hash:
        actual_sqlite_hash = sha256_file(args.sqlite)
        if actual_sqlite_hash != manifest_sqlite_hash:
            raise RuntimeError(
                "SQLite source changed after build: "
                f"manifest SHA256={manifest_sqlite_hash}, actual SHA256={actual_sqlite_hash}"
            )
    manifest_doc_hash = source.get("doc_sha256")
    if args.doc and manifest_doc_hash:
        actual_doc_hash = sha256_file(args.doc)
        if actual_doc_hash != manifest_doc_hash:
            raise RuntimeError(
                "DOC source changed after build: "
                f"manifest SHA256={manifest_doc_hash}, actual SHA256={actual_doc_hash}"
            )
