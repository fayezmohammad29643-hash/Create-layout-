...
def main() -> None:
    ...
    for expected_page, path in enumerate(page_files, start=1):
        ...
        for idx, line in enumerate(lines, start=1):
            ...
            for key in ("line", "text", "start", "end", "first_word", "end_word", "alignment", "runs"):
                if key not in line:
                    raise RuntimeError(f"{location}: missing {key}")
            if line["line"] != idx:
                raise RuntimeError(f"{location}: line number is {line['line']}, expected {idx}")
            if not isinstance(line["text"], str) or not line["text"].strip():
                raise RuntimeError(f"{location}: empty text")

            validate_endpoint(line["start"], f"{location} start")
            validate_endpoint(line["end"], f"{location} end")
            validate_endpoint(line["first_word"], f"{location} first_word")
            validate_endpoint(line["end_word"], f"{location} end_word")

            if line["first_word"] != line["start"]:
                raise RuntimeError(f"{location}: first_word must equal start")
            if line["end_word"] != line["end"]:
                raise RuntimeError(f"{location}: end_word must equal end")

            runs = line["runs"]
            if not isinstance(runs, list) or not runs:
                raise RuntimeError(f"{location}: runs must be a non-empty list")
            for ridx, run in enumerate(runs, start=1):
                run_loc = f"{location} run {ridx}"
                if not run.get("text"):
                    raise RuntimeError(f"{run_loc}: empty text")
                if run.get("font_family") != manifest_font:
                    raise RuntimeError(
                        f"{run_loc}: font_family={run.get('font_family')!r}, expected {manifest_font!r}"
                    )
