...
def _is_marker_text(text: str) -> bool:
    """Recognise the ayah-marker encoding after Unicode decoding."""
...
def _marker_char_info(chars: list[dict[str, Any]]) -> tuple[bool, set[int]]:
    """Detect a marker whose glyphs are split across multiple PDF spans.

    The Douri/Warsh source PDFs may expose one printed ayah marker as separate
    spans (typically left edge glyph + encoded digit + right edge glyph).  The
    span-level text is empty, so marker detection must operate on the combined
    rawdict line character stream.  The returned index set identifies the
    marker glyphs so they are not counted as Quran words.
    """
    compact_chars = [str(c.get("c", "")) for c in chars]
    edge_positions = [
        i for i, ch in enumerate(compact_chars)
        if ch in MARKER_EDGE_CHARS
    ]
    if len(edge_positions) < 2:
        return False, set()

    first = edge_positions[0]
    last = edge_positions[-1]
    if first >= last:
        return False, set()

    # A marker occupies a compact glyph run bounded by the two edge glyphs.
    # Require no whitespace glyphs inside it; this avoids classifying ordinary
    # Quran text that merely contains one marker edge glyph.
    inner = compact_chars[first:last + 1]
    if any(not ch.strip() for ch in inner):
        return False, set()
    if not ((inner[0] == "∩" and inner[-1] == "∪") or
            (inner[0] == "∪" and inner[-1] == "∩")):
        return False, set()

    return True, set(range(first, last + 1))
...
def _visual_units_from_raw_lines(
    ...
    *,
    ...
    for raw_line, spans in _raw_lines(page):
        ...
        # First detect a marker across the whole raw line.  This is essential
        # because a single printed marker can be split among HQPB/MSH spans.
        raw_chars: list[dict[str, Any]] = []
        for span in spans:
            raw_chars.extend(_span_chars(span))
        marker_found, marker_indices = _marker_char_info(raw_chars)
        raw_char_index = 0
        ...
        for span in spans:
            ...
            span_indices = set(range(raw_char_index, raw_char_index + len(chars)))
            raw_char_index += len(chars)
            span_is_marker = bool(marker_found and span_indices & marker_indices)

            if _font_is_quran(font):
                q_spans.append(span)
                if span_is_marker or _is_marker_text(text):
            ...
            # the HQPB span family.
            if span_is_marker or (
                any(hint in font for hint in MARKER_FONT_HINTS)
                and _is_marker_text(text)
            ):
        ...
        # A single printed ayah marker can span multiple rawdict spans.  The
        # line-level detector represents that compound object as exactly one
        # marker, while span-level fallback remains available for PDFs that
        # expose the marker as a single span.
        marker_count = 1 if marker_found else len(marker_spans)
